...
int main() {
    ...
    rep(i, n) {
        V1<ld> theta;
        rep(j, n) {
            if (i != j) theta.pb(arg(p[j] - p[i]));
        }
        sort(all(theta));
        // theta[i]は全て異なる値になっている(同じ値があった場合、直線ができてしまうので)
        int m = n - 1;
        rep(i, m) theta.pb(theta[i] + 2 * PI);
        ll r = 0;
        rep(l, m) {
            ld lim = theta[l] + PI / 2;
            while (r < l + m) {
                // 明らかにtheta[r]が大きくなったとき
                if (lim + EPS < theta[r]) break;
                // theta[l]からちょうどpi/2のところにtheta[r]がありそうなとき
                if (abs(lim - theta[r]) < EPS) break;
                r++;
            }
            if (abs(lim - theta[r]) < EPS) cho++;
            ei += r - 1 - l;
        }
    }
    ll san = n * (n - 1) * (n - 2) / 6;
    ll don = san * 3 - cho - ei;
    cout << san - don - cho << " " << cho << " " << don << endl;
    return 0;
}

/*
メモ
複素数を1つ決め、その複素数が原点に来るよう平行移動
各複素数の偏角をソートして、複素数を決めたときにそこからpi/2までの
範囲の複素数を尺取り法で数える
実装上、各偏角と+2piした偏角を両方持っておく
nがオーバーフローしていてペナルティを出した
*/
