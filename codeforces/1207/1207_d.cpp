...
int main() {
    ...
    vector<int> ca(M + 1, 0), cb(M + 1, 0);
    rep(i, N) ca[c[i].first]++, cb[c[i].second]++;
    vector<mint> fct(M, 1);
    for (int i = 1; i < M; i++) fct[i] = fct[i - 1] * i;
    mint cam = 1, cbm = 1;
    rep(i, M) {
        cam *= fct[ca[i]];
        cbm *= fct[cb[i]];
    ...
    rep(i, N) mp[c[i]]++;
    mint cabm = 1;
    for (auto p : mp) cabm *= fct[p.second];
    rep(i, N - 1) {
        if (c[i].second > c[i + 1].second) cabm = 0;
    }
    mint ans = 1;
