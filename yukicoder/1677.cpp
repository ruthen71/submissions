...
vll dfs(string &s, int ind, int &ind2) {
    show(ind);
    vll ret(3, 0);
    if (s[ind] == 'm') {
        int indm, indm2;
        // s[ind]=m s[ind+1]=a/e/? s[ind+2]=x s[ind+3]=(
        // s[ind+4]は次の要素の先頭の位置
        vll dfs1 = dfs(s, ind + 4, indm);
        // s[indm+1]=, s[indm+2]=次の要素の先頭の位置
        vll dfs2 = dfs(s, indm + 2, indm2);
        show(dfs1);
        show(dfs2);
        if (s[ind + 1] != 'a') {
            // mex
            rep(i, 3) rep(j, 3) {
                ret[mex(i, j)] += dfs1[i] * dfs2[j] % mod;
                ret[mex(i, j)] %= mod;
            }
        }
        if (s[ind + 1] != 'e') {
...
メモ
構文解析する
評価した式の答えが0,1,2になる場合の数を返すDFSをする
*/
